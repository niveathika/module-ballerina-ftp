...
function testPattern_TxtFilesDetected() returns error? {
    ...
    int txtSnapshot;
    boolean jsonSnapshot;
    lock { txtSnapshot = patternTxtCount; }
    lock { jsonSnapshot = patternJsonDetected; }

    test:assertTrue(txtSnapshot >= 1,
        "At least the .txt file should have been detected; got: " + txtSnapshot.toString());
    test:assertFalse(jsonSnapshot,
        "pattern-no-match.json must not be delivered by a .txt-only pattern listener");
...
// captured is declared with a readonly intersection because the FileInfo
// values delivered by the listener come from `WatchEvent & readonly` and
// readonly-immutable values can be safely transferred out of `lock` blocks.
isolated (ftp:FileInfo & readonly)? captured = ();
...
function testFileInfo_AllFields() returns error? {
    ...
    if fi is () {
        ...
        // path, uri and rootURI embed credentials via Apache VFS. Assert
        // structurally (scheme/host/port/path components) rather than with
        // full equality so a future masking fix for these fields would not
        // break the test, and so the suite does not hard-code the raw password.
        test:assertTrue(fi.path.startsWith(string `ftp://${user}:`) &&
                fi.path.endsWith(string `@${host}:${port}/home/in/test1.txt`),
            "FileInfo.path should start with ftp://${user}: and end with the remote path; got: " + fi.path);
        ...
        test:assertTrue(fi.uri.startsWith(string `//${user}:`) &&
                fi.uri.endsWith(string `@${host}:${port}/home/in/test1.txt`),
            "FileInfo.uri should start with //${user}: and end with the remote path; got: " + fi.uri);
        test:assertTrue(fi.rootURI.startsWith(string `ftp://${user}:`) &&
                fi.rootURI.endsWith(string `@${host}:${port}/`),
            "FileInfo.rootURI should start with ftp://${user}: and end with host:port/; got: " + fi.rootURI);
...
function testWatchEvent_DeletedFilesReported() returns error? {
    ...
    string namesSnapshot;
    lock { namesSnapshot = deletedNames; }
    foreach int i in 1...3 {
        test:assertTrue(namesSnapshot.includes(string `/home/in/del-test-${i}.del`),
