...
4. [Listener](#4-listener)
   ...
   * 4.3 [File Change Callbacks](#43-file-change-callbacks)
      * 4.3.1 [Generic File Change Callback](#431-generic-file-change-callback)
      * 4.3.2 [Format-Specific Callbacks](#432-format-specific-callbacks)
      * 4.3.3 [File Delete Callback](#433-file-delete-callback)
      * 4.3.4 [Error Callback](#434-error-callback)
...
- `put(path, content)` — A general-purpose write method that accepts a byte stream, a string, XML, or JSON. Supports optional compression via the `compressionType` parameter.
- `append(path, content)` — Appends a byte stream, a string, XML, or JSON to an existing file.
...
The `put()` method supports compressing content before uploading using the `compressionType` parameter. The supported compression type is `ZIP`. When `NONE` is specified (the default), no compression is applied.
...
#### 4.3.1 Generic File Change Callback
...
#### 4.3.2 Format-Specific Callbacks
